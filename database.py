...
def _run(fn):
    """Run fn(conn), reconnecting once if Turso's HTTP stream has gone stale.

    libSQL talks to Turso over short-lived HTTP streams; if the connection sits
    idle (e.g. during the ~minute of board-scraping between writes) the server
    drops the stream and the next call raises "stream not found". Reconnect and
    retry once so the pipeline doesn't crash.
    """
    global _conn
    try:
        return fn(get_connection())
...
def init_db():
    ...
    def op(conn):
        ...
        """)
        # Migrations for DBs created before these columns existed (no-ops otherwise).
        for column in (
            "emailed INTEGER NOT NULL DEFAULT 0",
            "eligible INTEGER NOT NULL DEFAULT 1",
            "date_posted TEXT",
...
def get_unscored_jobs():
    ...
    return _run(lambda c: _rows(c.execute("SELECT * FROM jobs WHERE score IS NULL")))
